...
def serialize_constituent(row: Constituent):
    return {
        ...
        "gender": row.gender,
        "phone": row.phone,
        "voting_district": row.voting_district,
        "district": row.voting_district,
        "historical_source_file": row.historical_source_file,
        "historical_source_year": row.historical_source_year,
        "historical_match_status": row.historical_match_status,
        "historical_enriched_at": serialize_dt(row.historical_enriched_at),
...
def constituents(subgroup: str = "", ward: str = "", q: str = "", limit: int = 250, _auth: AuthContext = Depends(require_staff_access), db: Session = Depends(get_db)):
    ...
    if q:
        ...
        query = query.filter(or_(
            ...
            Constituent.voting_district.ilike(term),
            Constituent.subgroup.ilike(term),
            Constituent.voter_status.ilike(term),
            Constituent.voter_id.ilike(term),
            Constituent.phone.ilike(term),
            Constituent.party_affiliation.ilike(term),
            Constituent.gender.ilike(term),
